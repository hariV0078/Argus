...
import { getDefaultBackendUrl, setBackendUrlValue } from '@/src/api/baseUrl';
import { connectBackend, getPipelineJob, listOutputRuns, normalizeStatus, runPipeline } from '@/src/api/client';
import { PipelineJob, RunOutputs, RunPipelineRequest } from '@/src/api/types';
import { loadServerConfig, saveServerConfig } from '@/src/lib/serverPersist';
...
export const useServerStore = create<ServerState>((set, get) => ({
  ...
  setBackendUrl: (backendUrl) => {
    ...
    void saveServerConfig({ url, epsg: get().utmEpsg });
  },
  setUtmEpsg: (utmEpsg) => {
    set({ utmEpsg });
    void saveServerConfig({ url: get().backendUrl, epsg: utmEpsg });
  },
  hydrate: async () => {
    ...
    // Placeholder sample data above renders instantly; try a real
    // connection in the background so a reachable backend replaces it
    // without the user having to visit Home/Settings and tap "Connect"
    // first (see refreshOutputs — same root cause).
    void get().connect();
  },
  connect: async () => {
  ...
  refreshOutputs: async () => {
    // Always attempt a real fetch first, regardless of `connected` — that
    // flag is only ever set by an explicit tap on the Home/Settings
    // "Connect" button, so opening Outputs directly (a fresh app launch,
    // or just not having visited those screens yet) always saw `connected
    // === false` and silently rendered fixed placeholder sample data
    // (mockRuns(), including its hardcoded Wisconsin map bounds) with no
    // indication it wasn't live, even with a perfectly reachable backend.
    // Self-heal `connected` on success so the rest of the UI (status
    // dot, etc.) reflects reality too.
    try {
      const runs = await listOutputRuns();
