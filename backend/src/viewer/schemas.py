...
class PipelineRequest(BaseModel):
    ...
    out_dir: Optional[str] = Field(
        None,
        description="Output directory for 3D model files. Defaults to a fresh "
                    "per-job directory (data/output/{job_id}) so each run's "
                    "outputs are kept — override only for advanced/manual use.",
    )
    ...
    skip_georef: bool = Field(
        False,
        description="No GPS available for this video (no DJI SRT, no embedded location). "
                    "Skips telemetry parsing, GPS alignment, and GeoTIFF export — mesh and "
                    "point cloud are produced in local (non-georeferenced) coordinates instead "
                    "of failing outright. utm_epsg/dsm_res_m are ignored when set.",
    )
...
class RunOutputs(BaseModel):
    """One completed (or in-progress) job's outputs, grouped by category —
    this is the unit the app renders as one "Run" card (mesh / point cloud / map)."""
    job_id: str
    created_at: Optional[str] = None
    status: Optional[str] = None
    mesh: list[OutputFileInfo] = Field(default_factory=list)
    pointcloud: list[OutputFileInfo] = Field(default_factory=list)
    geotiff: list[OutputFileInfo] = Field(default_factory=list)
...
class UploadResponse(BaseModel):
    video_path: str = Field(..., description="Server-side path — pass this straight into PipelineRequest.video_path")
    filename: str
    size_bytes: int
...
class SampleDataset(BaseModel):
    ...
    name: str
    description: str
    kind: str = Field(..., description="'frames' (pre-extracted, use skip_ingestion=true) or 'video'")
    frame_count: Optional[int] = None
    video_path: Optional[str] = None
    utm_epsg: int
    run_request: dict = Field(..., description="Suggested PipelineRequest body to submit as-is")
