...
def run(
    ...
    # Export all required formats (GeoTIFF DSM skipped if skip_georef — it
    # fundamentally requires real-world coordinates, which don't exist here)
...
def main():
    ...
    ap.add_argument("--skip-georef", action="store_true",
                     help="No GPS available — skip GPS alignment and GeoTIFF export; "
                          "mesh/point cloud are produced in local (non-georeferenced) coordinates.")
    args = ap.parse_args()
