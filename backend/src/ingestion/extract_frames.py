...
def extract_no_gps(
    ...
    """
    Same frame extraction as extract(), minus everything that needs GPS:
    no distance-based dedup (pure time-interval spacing via target_fps
    instead), no per-frame GPS/attitude columns (filled with NaN so the
    manifest keeps the same schema — align_to_gps / geotagging are simply
    skipped downstream when skip_georef is set, so nothing else reads them).

    Use for source video with no telemetry at all — the resulting mesh /
    point cloud will be in local (non-georeferenced) coordinates.
    """
    video_path = Path(video_path)
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    step = max(1, round(native_fps / target_fps))
    print(f"  [extract] {video_path.name}  native={native_fps:.2f}fps  "
          f"step={step} (→ ~{native_fps/step:.2f}fps sampled)  [no GPS — time-interval spacing]")
    ...
    with tqdm(total=total, unit="fr", desc="extracting") as pbar:
        while True:
            ...
            kept.append(dict(
                ...
                timestamp_ms=ts_ms,
                blur_score=score,
                lat=float("nan"), lon=float("nan"), alt_m=float("nan"), rel_alt_m=float("nan"),
                gimbal_pitch=float("nan"), gimbal_yaw=float("nan"), gimbal_roll=float("nan"),
            ))
            frame_idx += 1

    cap.release()

    print(f"  [extract] kept={kept_count}  blur_dropped={blur_dropped}  total_read={frame_idx}")

    if not kept:
        raise RuntimeError("No frames passed quality filters. Lower blur_threshold.")
